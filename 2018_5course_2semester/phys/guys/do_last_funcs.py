...
def do_map2():
    ...
    data = [ (i, sin(i/2) + cos(i /4)) for i in range(count)]
    data = np.array(data)
    return data
#==============
...
if __name__ == '__main__':
    ...
    #============== subplots ===========
    fig, axlist = plt.subplots(4,1)

    axlist[0].set_title("Cигнал: sin(i/2) + cos(i/4)")
    axlist[0].plot(x_array.T[1])

    axlist[1].set_title("AKF")
    axlist[1].plot(*data2.transpose())  #.T - it's numpy.transpose

    axlist[2].set_title("DFT")
    axlist[2].plot(data3)


    axlist[3].set_title("теорема Винера-Хинчина / DFT")
    axlist[3].plot(data3, label="DFT")

    axlist[3].plot(range(0,len(data)*2,2),data, label="Винер-Хинчин")
