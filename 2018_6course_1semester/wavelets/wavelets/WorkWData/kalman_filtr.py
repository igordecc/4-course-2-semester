...
Q=0.3;R=0.2;y=0;x=0#начальные дисперсии шумов(выбраны произвольно) и нулевые значения  переменных.
...
for i in np.arange(0,100,0.2):
                ...
                x=np.sin(i*0.2)#модельная функция для x.
                #y=1-exp(-1/T)+y*exp(-1/T)# модельная функция для y.
                y=np.sin(y)# модельная функция для y.
                ...
                Z.append(z+x)# накопление списка значений z.
